...
check_readme() {
    # Look for either README.md or README (prioritize README.md)
    if [ -f "README.md" ]; then
        README="README.md"
    elif [ -f "README" ]; then
        README="README"
    ...
    if (($(echo "$total == 0" | bc -l))); then
        ...
        echo "README lipsa."
        score=-5
    elif [ -f "$README" ] && [ "$(stat -c%s "$README")" -eq 0 ]; then
        echo "$README gol."
        score=-5
    else
        score=0
...
if [ $cnt -gt 0 ]; then
    ...
    total=$(bc <<< "if ($total < 0) 0 else $total")
...
# Clean junk
make -f Makefile_cpp clean &>/dev/null
make -f Makefile_java clean &>/dev/null
make -f Makefile_c clean &>/dev/null
make -f Makefile.PA $makefile clean &>/dev/null
rm -rf tmp
